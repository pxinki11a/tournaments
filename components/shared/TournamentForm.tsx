...
import { createTournament, updateTournament } from "@/lib/actions/tournaments.actions"
import { ITournaments } from "@/lib/database/models/tournaments.model"
import { tournamentDefaultValues } from "@/app/constants"
...
const TournamentForm = ({ userId, type, tournament, tournamentId }: TournamentFormProps) => {
  ...
  const initialValues = tournament && type === 'Update' 
    ? { 
      ...tournament, 
      startDateTime: new Date(tournament.startDateTime), 
      endDateTime: new Date(tournament.endDateTime) 
    }
    : tournamentDefaultValues;
    ...
    if(type === 'Create') {
      try {
        const newTournament = await createTournament({
          tournament: { ...values, imageUrl: uploadedImageUrl },
          userId,
          path: '/profile'
        })
    ...
    if(type === 'Update') {
      ...
      try {
        const updatedTournament = await updateTournament({
          userId,
          tournament: { ...values, imageUrl: uploadedImageUrl, _id: tournamentId },
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="w-full">
                <FormControl>
                  <Input placeholder="Название турнира" {...field} className="input-field" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
                ...
                <FormItem className="w-full">
                  <FormControl>
                    <div className="flex-center h-[54px] w-full overflow-hidden rounded-full bg-grey-50 px-4 py-2">
                      ...
                      <Input placeholder="Оффлайн/Онлайн турнир (Место)" {...field} className="input-field" />
                    </div>
