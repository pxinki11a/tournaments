...
const Orders = async ({ searchParams }: SearchParamProps) => {
  ...
  const orders = await getOrdersByTournament({ tournamentId, searchString: searchText })

  return (
    <>
      <section className=" bg-primary-50 bg-dotted-pattern bg-cover bg-center py-5 md:py-10">
        <h3 className="wrapper h3-bold text-center sm:text-left ">Участия</h3>
      </section>
                ...
                {orders &&
                  orders.map((row: IOrderItem) => (
                    <tr
                      ...
                      <td className="min-w-[200px] flex-1 py-4 pr-4">{row.tournamentTitle}</td>
